...
import { generateAgentName } from "../utils.js";
...
export default function piMeshExtension(pi) {
    ...
    pi.events.on("mesh:provideContract:request", async (data) => {
        ...
        try {
            const projectState = new ProjectStateStore(MESH_DIR);
            const state = projectState.load(params.cwd ?? process.cwd());
            const contracts = projectState.get(state, "contracts") ?? {};
    ...
    function startHeartbeat(agentId) {
        if (heartbeatTimers.has(agentId))
            return;
    ...
    function stopHeartbeat(agentId) {
...
async function handleMeshAction(params, ctx, deps) {
    ...
    switch (params.action) {
        ...
        case "leave": {
            if (!myAgentId)
                return { status: "error", message: "Not joined" };
            stopHeartbeat(myAgentId);
            reservations.releaseAll([myAgentId]);
            // Unclaim tasks
        ...
        case "send": {
            ...
            if (!params.to || !params.message)
                return { status: "error", message: "to and message required" };
            const me = registry.get(myAgentId);
            inbox.send({
                from: myAgentId,
                fromName: me.name,
        ...
        case "reserve": {
            ...
            if (!params.paths?.length)
                return { status: "error", message: "paths required" };
            const me = registry.get(myAgentId);
        ...
        case "claim":
            {
                ...
                if (!task)
                    return { status: "error", message: "Task not available" };
                // Reserve files
                ...
                if (task.roleId) {
                    ...
                    try {
                        ...
                        role = roles.find((r) => r.id === task.roleId);
                    }
                    catch { /* ignore */ }
                    if (role) {
    ...
    "unclaim";
    {
        ...
        if (task)
            reservations.release(myAgentId);
    ...
    "task_done";
    {
        ...
        if (task)
            reservations.release(myAgentId, task.files);
    ...
    {
        if (!params.ext)
            return { status: "error", message: "ext required" };
        const projectState = new ProjectStateStore(MESH_DIR);
        const state = projectState.load(ctx.cwd ?? process.cwd());
        projectState.set(state, params.ext, params.data ?? {});
        projectState.save(state);
    ...
    "contract_provide";
    {
        if (!myAgentId)
            return { status: "error", message: "Not joined" };
        if (!params.item)
            return { status: "error", message: "item required" };
        const projectState = new ProjectStateStore(MESH_DIR);
        const state = projectState.load(ctx.cwd ?? process.cwd());
        const contracts = projectState.get(state, "contracts") ?? {};
        contracts[params.item] = {
    ...
    "contract_need";
    {
        ...
        if (!found)
            return { status: "error", message: `Contract "${params.item}" not yet provided` };
